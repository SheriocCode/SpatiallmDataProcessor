...
import random
from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg  # 用于交互
...
def draw_subplot_6(ax, density_image, annotations_by_room, color_map):
    ax.imshow(density_image)
    for room_id, anns in annotations_by_room.items():
        ...
        for ann in anns:
            ...
            if is_line_annotation and len(ann['segmentation'][0]) >= 4:
                seg = ann['segmentation'][0]
                x1, y1, x2, y2 = seg[:4]
                ax.plot([x1, x2], [y1, y2], 
                        color=base_color, 
                ...
                if len(bbox) == 4 and bbox[2] > 0 and bbox[3] > 0:
                    ...
                    rect = plt.Rectangle((x, y), w, h, 
                                       ...
                                       linestyle='--')
                    ax.add_patch(rect)
    ax.set_title('6. 点云密度图与可视化标注', fontsize=12)
    ax.axis('off')

def draw_subplot_7(ax, density_image, triang, adjusted_ids, cmap, annotations_by_room, color_map):
    ax.imshow(density_image)
    ax.tripcolor(triang, adjusted_ids, cmap=cmap, alpha=0.3)
            ...
            if is_line_annotation and len(ann['segmentation'][0]) >= 4:
                ...
                if len(bbox) == 4 and bbox[2] > 0 and bbox[3] > 0:
                    ...
                    rect = plt.Rectangle((x, y), w, h, 
                                       ...
                                       linestyle='--')
                    ax.add_patch(rect)
    ax.set_title('7. 点云密度图与标注+三角剖分结果', fontsize=12)
    ax.axis('off')
...
def visualize_combined_results(json_path, density_map_path, output_ply="combined_mesh.ply"):
    ...
    subplot_data = [
        ...
        lambda ax: draw_subplot_6(ax, density_image, annotations_by_room, color_map),
        lambda ax: draw_subplot_7(ax, density_image, triang, adjusted_ids, cmap, annotations_by_room, color_map)
    ]
