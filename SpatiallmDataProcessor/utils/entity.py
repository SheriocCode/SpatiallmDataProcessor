...
class Door:
    id: int
    wall_id: int
    position_x: float      # 门中心 X (世界坐标)
    position_y: float      # 门中心 Y
    position_z: float      # 门底面 Z（通常 0）
    width: float           # 门洞宽度 (m)
    height: float          # 门洞高度 (m)
...
class Bbox:
    id: int
    class_name: str
    position_x: float      # 盒子中心 X (世界坐标)
    position_y: float      # 盒子中心 Y
    position_z: float      # 盒子中心 Z
    angle_z: float         # 绕 Z 轴的旋转角 (rad，右手系)
    scale_x: float         # 盒子在自身 X 方向的半边长 *2（即全长）
    scale_y: float         # 盒子在自身 Y 方向的半边长 *2
    scale_z: float         # 盒子在自身 Z 方向的半边长 *2
